...
func (s *SQLiteStore) GetHistoryTimeBounds(ctx context.Context, groupID string) (oldest, newest *time.Time, err error) {
	...
	err = s.db.QueryRowContext(ctx, query, groupID).Scan(&minTimeStr, &maxTimeStr)
	if err != nil {
		return nil, nil, fmt.Errorf("querying history time bounds: %w", err)
	}

	if minTimeStr.Valid && minTimeStr.String != "" {
		t, parseErr := time.Parse(time.RFC3339Nano, minTimeStr.String)
		if parseErr != nil {
			// Try alternative format
			t, parseErr = time.Parse("2006-01-02 15:04:05.999999999-07:00", minTimeStr.String)
		}
	...
	if maxTimeStr.Valid && maxTimeStr.String != "" {
		t, parseErr := time.Parse(time.RFC3339Nano, maxTimeStr.String)
		if parseErr != nil {
			// Try alternative format
			t, parseErr = time.Parse("2006-01-02 15:04:05.999999999-07:00", maxTimeStr.String)
		}
...
func (s *SQLiteStore) GetHistoryStats(ctx context.Context, opts HistoryStatsOpts) (*HistoryStatsResult, error) {
	...
	// We use SQLite's datetime functions to truncate to bucket boundaries.
	query := `
		SELECT
			CAST((strftime('%s', completed_at) - strftime('%s', ?)) / ? AS INTEGER) AS bucket_idx,
...
func (s *SQLiteStore) DeleteExpiredOAuthStates(ctx context.Context) error {
	_, err := s.db.ExecContext(ctx, `DELETE FROM oauth_states WHERE expires_at < ?`, time.Now())
	if err != nil {
...
func (s *SQLiteStore) DeleteAuthCode(ctx context.Context, code string) error {
	_, err := s.db.ExecContext(ctx, `DELETE FROM auth_codes WHERE code = ?`, code)
	if err != nil {
...
func (s *SQLiteStore) DeleteExpiredAuthCodes(ctx context.Context) error {
	_, err := s.db.ExecContext(ctx, `DELETE FROM auth_codes WHERE expires_at < ?`, time.Now())
	if err != nil {
