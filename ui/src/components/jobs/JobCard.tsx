...
export function JobCard({ job, template, isDragging, dragHandleProps }: JobCardProps) {
  ...
  const effectiveName = job.name ?? template?.name ?? (isManualJob ? 'Manual Job' : job.template_id);
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            {isManualJob && (
              <span className="rounded-sm bg-cyan-500/10 px-1.5 py-0.5 text-xs text-cyan-400" title="Manual job (no template)">
                Manual
              </span>
            )}
            {!isManualJob && matchesTemplate() && (
          ...
          {/* Labels */}
          {(template?.labels && Object.keys(template.labels).length > 0) || (job.labels && Object.keys(job.labels).length > 0) ? (
            <div className="mt-1">
              <LabelsDisplay labels={job.labels ?? template?.labels ?? {}} maxDisplay={3} />
            </div>
          ) : null}
      ...
      <div className="mt-3 flex items-center justify-between gap-4 text-xs text-zinc-500">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
          {(effectiveOwner && effectiveRepo) && (
            <span className={`flex items-center gap-1 ${(!isManualJob && (job.owner || job.repo)) ? 'text-amber-400' : ''}`} title={(!isManualJob && (job.owner || job.repo)) ? 'Repository overridden' : undefined}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full max-w-sm mx-4 rounded-sm border border-zinc-800 bg-zinc-900 shadow-xl">
            <div className="p-4">
              ...
              {job.auto_requeue && (
                <div className="flex items-start gap-2 rounded-sm bg-amber-500/10 border border-amber-500/20 p-3 mb-4">
                  ...
                  <p className="text-sm text-amber-400">
                    This job has auto-requeue enabled. It will automatically reappear in the queue after cancellation.
                  </p>
                </div>
              )}
              <div className="flex justify-end gap-2">
